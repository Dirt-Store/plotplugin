...
public class SignListener implements Listener {
    ...
    public void onPlayerInteract(PlayerInteractEvent event) {
        ...
        if (plugin.getPlotManager().hasPendingSignRegistration(playerId)) {
            ...
            // Remove old index entries before updating locations
            ...
            // Re-add with updated locations
            plugin.getSignManager().indexPlot(plot);
        ...
        if (!plot.isOwned()) {
            // Enforce the per-player plot limit before even opening the buy GUI
            ...
            if (maxP > 0 && !player.hasPermission("plotdirt.bypass-limit")) {
                if (plugin.getPlotManager().countOwnedPlots(player.getUniqueId()) >= maxP) {
                    player.sendMessage(plugin.getConfigManager().getMessage(
                            "plot-limit-reached", "{max}", String.valueOf(maxP)));
                    return;
                }
            }
