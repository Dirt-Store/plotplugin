...
public class PlotCommand implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (sub) {
            ...
            case "limit"         -> cmdLimit(sender, args);
    ...
    // ── /plot limit <name> <amount> ───────────────────────────────────────────

    private void cmdLimit(CommandSender sender, String[] args) {
        if (!sender.hasPermission("plotdirt.limit")) {
            sender.sendMessage(msg("no-permission"));
            return;
        }
        if (args.length < 3) {
            sender.sendMessage(raw("§cUsage: §e/plot limit §7<name> <amount>"));
            return;
        }
        String name = args[1];
        Plot plot = plugin.getPlotManager().getPlot(name);
        ...
        try {
            ...
            sender.sendMessage(raw("§c'" + args[2] + "' is not a valid number."));
            return;
        }
        if (amount < 1) {
            sender.sendMessage(raw("§cLimit must be at least 1."));
            return;
        }

        plot.setMemberLimit(amount);
        plugin.getPlotManager().savePlotAsync(plot);
        plugin.getSignManager().updateSign(plot);
        sender.sendMessage(msg("plot-limit-set", "{name}", name, "{limit}", String.valueOf(amount)));
    }
    ...
    private void cmdTransfer(CommandSender sender, String[] args) {
        ...
        if (transferWorld != null) {
            if (plugin.getConfigManager().isClearMembersOnTransfer()) {
                // Clear all old members/managers from WG, then add only the new owner.
                plugin.getWorldGuardManager().resetPlotRegion(transferWorld, name);
                plugin.getWorldGuardManager().grantPlayerAccess(transferWorld, name, target.getUniqueId());
                // Also clear them from the plot model
                plot.getMembers().clear();
                plot.getManagers().clear();
            } else {
                // Keep existing members/managers — only swap the owner in WG.
                if (oldOwner != null) {
                    plugin.getWorldGuardManager().revokePlayerAccess(transferWorld, name, oldOwner);
                }
                plugin.getWorldGuardManager().grantPlayerAccess(transferWorld, name, target.getUniqueId());
            }
        }
        ...
        // Evict old owner name cache since they no longer own this plot
    ...
    private void cmdAddMember(CommandSender sender, String[] args) {
        ...
        // Role protection: owners cannot add themselves as member
        if (plot.isOwner(targetId)) {
            player.sendMessage(msg("cannot-add-owner-as-member")); return;
        }

        // Role protection: managers cannot add themselves as member
        if (plot.isManager(player.getUniqueId()) && targetId.equals(player.getUniqueId())) {
            player.sendMessage(msg("cannot-add-self-as-member")); return;
        }
        ...
        // Member limit check
        int limit = plugin.getConfigManager().getEffectiveMemberLimit(plot);
        if (plot.getTotalPlayerCount() >= limit) {
            player.sendMessage(msg("member-limit-reached",
                    "%count%", String.valueOf(plot.getTotalPlayerCount()),
    ...
    private void cmdAddManager(CommandSender sender, String[] args) {
        ...
        // Role protection: owners cannot add themselves as manager
        if (plot.isOwner(targetId)) {
            player.sendMessage(msg("cannot-add-owner-as-manager")); return;
        }
        ...
        // Member limit check — only if the player isn't already counted (not a member)
        boolean alreadyMember = plot.isMember(targetId);
        if (!alreadyMember) {
            int limit = plugin.getConfigManager().getEffectiveMemberLimit(plot);
            if (plot.getTotalPlayerCount() >= limit) {
                player.sendMessage(msg("member-limit-reached",
    ...
    private void cmdReload(CommandSender sender) {
        ...
        // BUG FIX RELOAD-1: Save current in-memory state before reloading so
        // any unsaved changes (e.g. a plot just purchased) are not lost.
        plugin.getPlotManager().save();

        // BUG FIX RELOAD-2: Reload plots from DB so the in-memory map matches
        // what is on disk. Without this, the timer task and GUI work on stale
        // Plot objects after a reload, causing the "broken plot / kicked from
        // plugin" symptom: the old Plot reference no longer exists in the map
        // so look-ups return null and the GUI / WG sync throws NPEs.
        plugin.getPlotManager().load();

        // Reload config + GUIs + signs (existing behaviour)
        plugin.getConfigManager().reload();

        sender.sendMessage(raw("§aPlotDirt config and plot data reloaded successfully."));
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
        if (args.length == 1) {
            List<String> subs = new ArrayList<>(List.of(
                    ...
                    "setcoin", "paused", "unpaused", "whitelist", "unwhitelist", "reload", "limit"));
    ...
    private void sendUsage(CommandSender sender) {
        ...
        sender.sendMessage(raw("§b/plot limit <name> <amount> §7- Set member limit for a plot"));
