...
public class PlotCommand implements CommandExecutor, TabCompleter {
    ...
    private void cmdBuy(CommandSender sender, String[] args) {
        ...
        if (!confirmed) {
            ...
            // Check limit before even opening the buy GUI
        ...
        // ── Purchase lock — blocks duplicate clicks before the first buy commits ──
        if (!plugin.getPlotManager().tryAcquirePurchaseLock(player.getUniqueId())) {
            // A purchase is already being processed for this player; silently ignore
            return;
        }
        // ─────────────────────────────────────────────────────────────────────────
        ...
        try {
            ...
            // Always release the lock so the player is not permanently blocked
