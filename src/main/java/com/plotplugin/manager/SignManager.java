...
public class SignManager {
    ...
    public void updateSign(Plot plot) {
        ...
        // السطر الأول: اسم البلوت دائماً بلون سماوي
        String line0 = "&b" + plot.getName();

        String line1; // الحالة
        String line2; // المالك أو السعر
        String line3; // الوقت المتبقي

        switch (plot.getState()) {
            case AVAILABLE -> {
                line1 = cfg.getSignLine("line-available");
                line2 = cfg.getSignLine("line-price", "{price}", String.valueOf(cfg.getBuyCost()));
            ...
            case PAUSED -> {
                ...
                line2 = cfg.getSignLine("line-owner", "{owner}", ownerName != null ? ownerName : "Unknown");
                line3 = cfg.getSignLine("line-time", "{time}", TimeUtil.format(plot.getRemainingMs()));
            }
            default -> { // OWNED
        ...
        side.line(0, toComponent(line0));
        side.line(1, toComponent(line1));
        side.line(2, toComponent(line2));
        side.line(3, toComponent(line3));
    ...
    public void scheduleSignRestore(Plot plot) {
        Bukkit.getScheduler().runTaskLater(plugin, () -> {
            ...
            if (block.getType().isAir()) {
                // sign is gone — listener handles prevention; nothing to do here
            }
            updateSign(plot);
        }, 1L);
    }
