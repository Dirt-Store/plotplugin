...
public class PlotCommand implements CommandExecutor, TabCompleter {
    ...
    private void cmdBuy(CommandSender sender, String[] args) {
        ...
        // Grant WG member access (owner is stored as WG member so flag ALLOW-MEMBERS applies)
        for (World world : Bukkit.getWorlds()) {
            if (plugin.getWorldGuardManager().regionExists(world, name)) {
        ...
        plugin.getPlotManager().savePlotAsync(plot); // async — avoid main-thread I/O
    ...
    private void cmdReload(CommandSender sender) {
        if (!sender.hasPermission("plotplugin.reload")) {
            sender.sendMessage(msg("no-permission"));
            return;
        }
        plugin.getConfigManager().reload();
        sender.sendMessage("§aPlotPlugin config reloaded successfully.");
    }
    ...
    private void sendUsage(CommandSender sender) {
        ...
        sender.sendMessage("§b/plot reload §7- Reload config");
    }
}
