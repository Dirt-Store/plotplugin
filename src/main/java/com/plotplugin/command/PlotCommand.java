...
public class PlotCommand implements CommandExecutor, TabCompleter {
    ...
    private void cmdBuy(CommandSender sender, String[] args) {
        ...
        // Grant WG owner access (plot owner → WG owner for full region control)
        for (World world : Bukkit.getWorlds()) {
            if (plugin.getWorldGuardManager().regionExists(world, name)) {
    ...
    private void cmdTransfer(CommandSender sender, String[] args) {
        if (!(sender instanceof Player player)) { sender.sendMessage("&cPlayers only."); return; }
        if (args.length < 2) { sender.sendMessage("&cUsage: /plot transfer <name> [player]"); return; }
        ...
        if (!plot.isOwner(player.getUniqueId()) && !player.hasPermission("plotplugin.admin")) {
            player.sendMessage(msg("not-owner"));
            return;
        }

        // No target name yet — prompt via chat
        if (args.length < 3) {
            promptChatInput(player, "transfer", name);
            return;
        }
        ...
        if (!target.hasPlayedBefore() && !target.isOnline()) {
            player.sendMessage(msg("player-not-found", "{player}", targetName));
            return;
        }

        UUID newOwnerUuid = target.getUniqueId();

        if (newOwnerUuid.equals(plot.getOwner())) {
            player.sendMessage(msg("already-owner", "{player}", targetName));
            return;
        }

        UUID oldOwnerUuid = plot.getOwner();

        // Remove the incoming owner from any existing role before promoting them
        plot.removeMember(newOwnerUuid);
        plot.removeManager(newOwnerUuid);

        // The old owner is removed from the plot entirely (clean slate)
        plot.setOwner(newOwnerUuid);

        // Sync WG: rebuild region membership from scratch to match the new state
        for (World world : Bukkit.getWorlds()) {
            if (plugin.getWorldGuardManager().regionExists(world, name)) {
                // Revoke old owner's WG access
                if (oldOwnerUuid != null) {
                    plugin.getWorldGuardManager().revokePlayerAccess(world, name, oldOwnerUuid);
                }
                // Grant new owner WG owner access
                plugin.getWorldGuardManager().grantOwnerAccess(world, name, newOwnerUuid);
                // Also remove the new owner from WG members list (they're now an owner)
                plugin.getWorldGuardManager().getRegionManager(world)
                      .getRegion(name).getMembers().removePlayer(newOwnerUuid);
                break;
            }
        }
    ...
    private void cmdAddMember(CommandSender sender, String[] args) {
        ...
        if (plot.isManager(targetId)) {
            player.sendMessage(msg("already-manager", "{player}", targetName));
            return;
        }
        if (plot.isMember(targetId)) {
            player.sendMessage(msg("already-member", "{player}", targetName));
            return;
        }
    ...
    private void cmdRemoveMember(CommandSender sender, String[] args) {
        ...
        if (plot.isManager(player.getUniqueId()) && !plot.isOwner(player.getUniqueId())) {
            if (plot.isOwner(targetId) || plot.isManager(targetId)) {
                player.sendMessage(msg("cannot-manage-owner"));
                return;
            }
        }

        if (!plot.isMember(targetId)) {
            player.sendMessage(msg("not-member", "{player}", targetName));
            return;
        }
    ...
    private void cmdAddManager(CommandSender sender, String[] args) {
        ...
        // PROMOTION FIX: if the player was a member, remove them from the member list
        // before promoting so they appear exactly once (as manager) in the GUI.
        boolean wasMember = plot.isMember(targetId);
        if (wasMember) {
            plot.removeMember(targetId);
            // WG: their member entry stays — we keep the WG member access and don't need to re-add
        }

        plot.addManager(targetId);

        if (!wasMember) {
            // They weren't already a WG member; grant access now
            for (World world : Bukkit.getWorlds()) {
                if (plugin.getWorldGuardManager().regionExists(world, plotName)) {
                    plugin.getWorldGuardManager().grantPlayerAccess(world, plotName, targetId);
                    break;
        ...
        // If wasMember, they're already in WG members — no change needed for WG access
    ...
    private void cmdRemoveManager(CommandSender sender, String[] args) {
        ...
        if (!plot.isManager(targetId)) {
            player.sendMessage(msg("not-manager", "{player}", targetName));
            return;
        }
    ...
    private void cmdSetCoin(CommandSender sender, String[] args) {
        ...
        if (item.getType().isAir()) {
            player.sendMessage(msg("no-item-in-hand"));
            return;
        }
    ...
    private void promptChatInput(Player player, String action, String plotName) {
        ...
        String msgKey = action.equals("transfer") ? "input-transfer-name" : "input-member-name";
        player.sendMessage(msg(msgKey));
    }
